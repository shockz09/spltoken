...
import { PROGRAM_ID, DataV2, createCreateMetadataAccountV3Instruction, AuthorityType } from '@metaplex-foundation/mpl-token-metadata';
import { bundlrStorage, Metaplex, MetaplexFileTag, walletAdapterIdentity } from '@metaplex-foundation/js';

export async function createSPLToken(owner: PublicKey, wallet: WalletContextState, connection: Connection, quantity: number, decimals: number, isChecked: boolean, tokenName: string, symbol: string, metadataURL: string, description: string, wltAdr: string, file: Readonly<{
  buffer: Buffer;
  fileName: string;
  displayName: string;
  uniqueName: string;
  ...
  try {
    setIscreating(true)
    setTokenAddresss('')
    const connection = new Connection("enter ur rpc")
